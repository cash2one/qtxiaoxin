...
<div class="box">
    <div class="form tableBox">
        <form id="business-form" action="<?php echo Yii::app()->createUrl("student/saveexcel");?>" method="post">
        <table class="tableForm">
            ...
            <tbody>  
                    ...
                    <td>
                        <div style="display: inline;">
                             <select id="schoolId" name=" " datatype="*" nullmsg="请选择学校！">
                                 ...
                                 <?php if(is_array($schools)) foreach($schools as $key=>$val):?>
                ...
                <tr>
                    ...
                    <td>
                        <div  class="createBox" style="display:none; height: 420px; overflow-x:hidden; overflow-y:hidden;">
                            <table class="tableForm">
                               <thead></thead>
                               <tbody>
                                   <tr>
                                       <td>
                                            ...
                                            <span id="Validform_file"  class="Validform_checktip ">&nbsp;&nbsp;<a  href="<?php echo Yii::app()->request->baseUrl.'/template/学生导入模板.xls'; ?>">下载格式模版 </a></span>
                                   ...
                                   <tr class="msghmmTr" style="display:none">
                                       <td><div style="padding-left: 30px;"><input id="msghmm" type="checkbox" name="issendpwd" value="1" style="margin-right:5px;"><label style="margin-top: 0px;" for="msghmm">发送账号密码通知短信</label></div></td>
                                       <td></td> 
                                   </tr>
                                   <tr class="msghmmCentenTr" style="display: none;">
                                       <td >
                                           短信内容：
                                           <textarea maxlength="250" style="color: #606060" name="smscontent">
                                               你好！感谢您使用<?php echo SITE_NAME;?>（<?php echo SITE_URL;?>）,平台有家校沟通丶成绩管理等功能。您的账号:[xxxxxxxxxxx]，密码:[xxxxxx]。客服电话:4001013838，工作时间:08:00-20:00
                                           </textarea><p style="margin-left:70px;">(请不要删除xxxxxx占位符，否则用户收不到登录帐号和密码)</p>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hidden" id="schoolid_hidden" name="schoolid" value="">
...
function ajaxFileUpload(){
    ...
    var ajaxurlUpload = '<?php echo Yii::app()->createUrl('/student/importstudent');?>'+'?schoolid='+schoolid;
    $('#result').empty();
    $.ajaxFileUpload({
        ...
        success: function (data, status) { //相当于java中try语句块的用法 
            ...
            if(arr.status=='1'){ //成功
                ...
                var shtml='<a id="fileName" class="fileIoc" href="javascript:void(0);">'+arr.filename+' </a>  一共：'+
                    (arr.validnum+arr.novalidnum)+' 条数据，'+arr.validnum+' 条符合，<span class="remind" style="color: red;"> '+arr.novalidnum+' 条无效。</span>';
                if(arr&&arr.t1&&arr.t1.length>0){
                    shtml+="<p style='padding-left: 30px;'>班级不存在行："+arr.t1.join("，")+"</p>";
                }
                if(arr&&arr.t2&&arr.t2.length>0){
                    shtml+="<p style='padding-left: 30px;'>手机格式错误行："+arr.t2.join("，")+"</p>";
                }
                if(arr&&arr.t3&&arr.t3.length>0){
                    shtml+="<p style='padding-left: 30px;'>姓名格式错误行："+arr.t3.join("，")+"</p>";
                }
