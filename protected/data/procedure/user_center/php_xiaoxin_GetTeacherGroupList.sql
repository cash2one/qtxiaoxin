...
DROP PROCEDURE IF EXISTS `php_xiaoxin_GetTeacherGroupList`$$

CREATE DEFINER=`dbManager`@`%` PROCEDURE `php_xiaoxin_GetTeacherGroupList`(p_creater BIGINT,p_type INT)
BEGIN
	...
	SET @where = CONCAT('WHERE `tb_group`.`deleted`=0 AND `tb_group`.`creater`=',p_creater); 	 
	IF (p_type!=-1) THEN 
