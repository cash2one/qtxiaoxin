...
{
	// $controller->layout = 'application.modules.xiaoxin.views.activity.main';
	/**
    ...
    public function accessRules()
    {
        return array(
            ...
            array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions' => array('view','index','rule','list','morelist','movieinfo','prize','draw','address'),
                'users' => array('@'),
            ),
	...
	{
		$this->renderPartial('index',array('url'=>Yii::app()->createUrl('xiaoxin/activity/view')));
	}
	...
	public function actionPrize()
	{
		...
		foreach ($prizes as $prize) {
			...
			if($goodName == '移动电源' || $goodName == 'IPHONE6'){
				...
				$mo = $prize->mo;
				if(empty($mo->mcaid)){
					$result['address']= Yii::app()->createUrl('xiaoxin/activity/address',array("Userid"=>$userid,"Mcaid"=>'',"Mogrid"=>$prize['mogrid']));
				}else{
					$result['address']= Yii::app()->createUrl('xiaoxin/activity/address',array("Userid"=>$userid,"Mcaid"=>$mo->mcaid));
				}
			}
			if($goodName == '电影通兑券'){
				$result['type'] = 3;
				$result['address']= Yii::app()->createUrl('xiaoxin/activity/movieinfo',array("Userid"=>$userid));
			}
	...
	public function actionAddress()
	{
		...
		$this->renderPartial('address',array('address'=>$address,'userid'=>$userid));
	}
