...
<div class="mianBox"> 
    <div id="submenuBoxR" class="submenuBoxR" data-viwe="show" >
        ...
        <div class="paneBox" style="background: #FFFFFF;">
            ...
            <!--<div class="pheader"><em class="Pico"></em>选择班级</div>-->
            <div class="groupBox" style=" display: none;">
                <?php if(is_array($schoolList)) { foreach($schoolList as $val):?>
                <div class="titleName" sid="<?php echo $val['sid'];?>"><em class=""></em><?php echo $val['name'];?></div>
                <ul id="school_<?php echo $val['sid'];?>" class="homeClassList">
                    ...
                    <?php if(is_array($val['class'])):?>
                       <?php foreach($val['class'] as $v):?>
                        <li><a rel="addClass" class="schoolT_<?php echo $val['sid'];?>" id="addClass_<?php echo $v['cid'];?>" tid="0" data-tpyid="1" data-cname="<?php echo $v['name'];?>" data-sid="<?php echo $val['sid'];?>" data-cid="<?php echo $v['cid'];?>" href="javascript:void(0);"><span class="fright ">✔</span><?php echo $v['name'];?></a></li>
                        <?php endforeach;?>
                    <?php endif;?>
                   <!-- <li><a rel="addClass" tid="0" data-uid="" data-cid="" href="javascript:void(0);">初二（2）班<span class="fright">✔</span></a></li>-->
                </ul>
                <?php endforeach;}?>
            </div>
        </div>
        ...
        <div class="box">
            <div class="headBox ">
                <div class="headNav noticeType_select">
                    <ul>
                        <?php if($noticeType==1):?>
                        <li><a avalue="1" href="<?php echo Yii::app()->createUrl("xiaoxin/notice/homework?noticeType=$noticeType");?> " class="focus" >班级</a></li>
                        <li><a avalue="2" href="<?php echo Yii::app()->createUrl("xiaoxin/notice/homework?receiveType=1&noticeType=$noticeType");?> " >分组</a></li>
                        <li><a avalue="0" href="<?php echo Yii::app()->createUrl("xiaoxin/notice/homework?receiveType=2&noticeType=$noticeType");?>" >学生</a></li>
                        <?php else:?>
                            <li><a avalue="1" href="<?php echo Yii::app()->createUrl("xiaoxin/notice/noticefamily?noticeType=$noticeType");?> " class="focus" >班级</a></li>
                            <li><a avalue="2" href="<?php echo Yii::app()->createUrl("xiaoxin/notice/noticefamily?receiveType=1&noticeType=$noticeType");?> " >分组</a></li>
                   ...
                   <table class="tableForm">
                       ...
                       <tr>
                            <td>
                                <div class="memberBox">
                                    <ul id="memberList">
                                        <li class="memberBtn"><a rel="addUserBtn" href="javascript:void(0);" ><em class="addBtnIco"></em> 添加班级</a></li> 
                                    </ul>
                                </div>
                                <div id="cuntUserCheck" class="cuntMember" >已选择了<span class="red">0</span>个班级<span id="cuntTip" style="display: none;" class="Validform_checktip Validform_wrong">至少添加一个班级</span></div>
                        ...
                        <tr>
                            <td>
                                ...
                                <div style=" ">
                                    <!--<div id="textareaCont" class="textFixed" style="width:auto; height: 120px; border:1px solid #E1E1E1; padding: 3px 8px 5px; outline: none;" contenteditable="true" contentEditable="true" placeholder="请在这里填写发送内容"></div>-->
...
<div id="previewMsg">
    <div id="previewMsgBox" class="popupBox" style="height: 636px;background-color:none;background:url(<?php echo Yii::app()->request->baseUrl; ?>/image/xiaoxin/preview-msg.png) no-repeat center center;">
...
<div id="selectClassBox">
    <div id="selectClass" class="popupBox" style="width: 640px; height: 495px; *height: 515px;" >
        <div class="header" style="">添加班级<a href="javascript:void(0);" class="close" onclick="hidePormptMaskWeb('#selectClass')" ></a></div>
...
<script type="text/javascript">
    ...
    $('[rel=chekedItime]').live('click',function(){ 
        ...
        if(parseInt(type)==0){ //选中
            if($("#checkboxs_"+sid+"_"+cid).length>0){ //是否保存过
                $(this).parent('li').attr('severtype','0'); //保存过 
            }else{
                $(this).parent('li').attr('severtype','1'); //没保存过  
            } 
            $(this).attr('tip',1);
            ...
            if($("#checkboxs_"+sid+"_"+cid).length>0){ //是否保存过
                $(this).parent('li').attr('severtype','2'); //保存过   取消
            }else{
                $(this).parent('li').attr('severtype','0');//没保存过  取消
            } 
            $(this).attr('tip','0');
    ...
    $('#saveMemberBtn').live('click',function(){
        ...
        //上传图片预览
        $('#plupload .view').click(function(){
            var imgUrl = $(this).find('img').attr('src'); 
            var img ='<img maxwidth="800px"  src="'+imgUrl+'">';
            $("#imgPBox").empty();
            $("#imgPBox").append(img);
        ...
        $('[rel=addAllClass]').live('click',function(){
            ...
            var len =$('#memberList').find('li').length-1; 
            if(parseInt(len)>0){
                $('#cuntTip').hide();
            }
            $("#cuntUserCheck").find('span.red').text(len);
