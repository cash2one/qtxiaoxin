...
<div class="header">
    ...
    <div class="titleText"><em class="inIco3"></em>日常工作 > 通知老师</div>
</div>
<div class="mianBox">
    <div id="submenuBoxR" class="submenuBoxR" data-viwe="show" >
        ...
        <div class="paneBox" style="background: #FFFFFF;">
            ...
            <div class="box" style="padding: 15px 0;">可以向学校老师发布学校通知。</div>
            ...
            <div class="groupBox" style="position: relative; display: none;">
                <div class="titleName selectTite">
                    <a rel="showSchoolClass" tid="0" href="javascript:void(0);">选择学校部门/组</a>
                </div>
                <ul id="listSc" class="listSc" style="position: absolute; display: none; overflow-x: hidden;overflow-y:auto; max-height: 650px; *height: 580px;"> 
                    <?php if(is_array($schoolList)){?>
                        <?php foreach($schoolList as $k=>$v):?>
                            ...
                            <li><a rel="selectClass" href="javascript:void(0);"  sid="<?php echo $v['sid'];?>" sname="<?php echo $v['name'];?>" cname="全体老师" type="school" cid="<?php echo $v['sid'];?>" class="class selectClass">全体老师</a></li>
                            <?php if(is_array($v['depts'])){ foreach($v['depts'] as $t):?>
                                <li><a rel="selectClass" sid="<?php echo $v['sid'];?>" type="dept" sname="<?php echo $v['name'];?>" cname="<?php echo $t['name'];?>" cid="<?php echo $t['did'];?>" href="javascript:void(0);" class="class selectClass"><?php echo $t['name'];?> </a></li>
                            <?php endforeach;?> 
                            <?php };?>
                            <?php if(is_array($v['groups'])){ foreach($v['groups'] as $t):?>
                                <li><a rel="selectClass" sid="<?php echo $v['sid'];?>" type="group" sname="<?php echo $v['name'];?>" cname="<?php echo $t['name'].'(分组)';?>" cid="<?php echo $t['gid'];?>" href="javascript:void(0);" class="class selectClass"><?php echo $t['name'].'(分组)';?> </a></li>
                            <?php endforeach;?>
                            <?php };?>
                        <?php endforeach;}?>
                </ul>
                    ...
                    <table class="tableForm">
                        ...
                        <tr>
                            <td>
                                <div class="memberBox">
                                    <ul id="memberList">
                                        <li class="memberBtn"><a rel="addUserBtn" href="javascript:void(0);" ><em class="addBtnIco"></em> 添加老师</a></li> 
                                    </ul>
                                </div>
                            ...
                            <td>
                                ...
                                <div style="padding: 10px 0;">
                                    <label class="checkbox">
                                     <input name="isSendToMe" value="1" type="checkbox">给自己发送一条确认短信</label>
...
<div id="selectClassBox">
    <div id="selectClass" class="popupBox" style="width: 640px; height: 495px; *height: 515px;" >
        <div class="header" style="">添加老师<a href="javascript:void(0);" class="close" onclick="hidePormptMaskWeb('#selectClass')" ></a></div>
        <div id="selectContent" class="popupContent" style="padding: 20px;">
            <div class="selectBox">
                ...
                <select id="selectClassListBox_<?php echo $v['sid'];?>" class="selectClassListBox" style=" display: none; width: 200px;">
                    <option tip="0" value="">选择部门/分组</option>
                    <option tip="0" value="<?php echo $v['sid'];?>" rel="selectClass" sid="<?php echo $v['sid'];?>" sname="<?php echo $v['name'];?>" cname="全体老师" type="school" cid="<?php echo $v['sid'];?>" >全体老师</option>
                    <?php if(is_array($v['depts'])){ foreach($v['depts'] as $t):?>
                        <option tip="0"  sid="<?php echo $v['sid'];?>" type="dept" sname="<?php echo $v['name'];?>" cname="<?php echo $t['name'];?>" cid="<?php echo $t['did'];?>" ><?php echo $t['name'];?> </option>
                    <?php endforeach;};?>  
                    <?php if(is_array($v['groups'])){ foreach($v['groups'] as $t):?>
                       <option tip="0" sid="<?php echo $v['sid'];?>" type="group" sname="<?php echo $v['name'];?>" cname="<?php echo $t['name'].'(分组)';?>" cid="<?php echo $t['gid'];?>" ><?php echo $t['name'].'(分组)';?> </option>
                    <?php endforeach; };?> 
                </select>
                    <a rel="allCheck" sid="<?php echo $v['sid'];?>" id="clear_<?php echo $v['sid'];?>_school_<?php echo $v['sid'];?>" type="school" cid="<?php echo $v['sid'];?>_school_<?php echo $v['sid'];?>" class="clear" tip="0" href="javascript:;" style=" display: none;">全部选中</a>
                    <?php if(is_array($v['depts'])){ foreach($v['depts'] as $t):?>
                        <a rel="allCheck" sid="<?php echo $v['sid'];?>" id="clear_<?php echo $t['sid'];?>_dept_<?php echo $t['did'];?>" type="dept" cid="<?php echo $v['sid'];?>_dept_<?php echo $t['did'];?>" class="clear" tip="0" href="javascript:;" style=" display: none;">全部选中</a>
                         <?php endforeach;};?>  
                    <?php if(is_array($v['groups'])){ foreach($v['groups'] as $t):?>
                        <a rel="allCheck" sid="<?php echo $v['sid'];?>" id="clear_<?php echo $v['sid'];?>_group_<?php echo $t['gid'];?>" type="group" cid="<?php echo $v['sid'];?>_group_<?php echo $t['gid'];?>" class="clear" tip="0" href="javascript:;" style=" display: none;">全部选中</a>
                    <?php endforeach;};?>
                <?php endforeach;};?>
            </div>
            <div id="memberBoxId" class="memberBox" style="height: 305px; margin-bottom: 15px;">
                ...
                <?php foreach($schoolList as $k=>$v):?>
                    <div rel="removeListClass" id="schoolCalssListBox_<?php echo $v['sid'];?>" class="divUlListBox">
                        <ul id="popMember_<?php echo $v['sid'];?>_school_<?php echo $v['sid'];?>" class="ulListBox"> 
                        </ul>
                          <?php if(is_array($v['depts'])){ foreach($v['depts'] as $t):?>
                            <ul  id="popMember_<?php echo $v['sid'];?>_dept_<?php echo $t['did'];?>" class="ulListBox"> 
                            </ul>
                            <?php endforeach;};?>
                            <?php if(is_array($v['groups'])){ foreach($v['groups'] as $t):?>
                               <ul id="popMember_<?php echo $v['sid'];?>_group_<?php echo $t['gid'];?>" class="ulListBox"> 
                               </ul>
                           <?php endforeach; };?> 
                    </div>
                <?php endforeach;};?>
...
<script type="text/javascript">
    $(function(){
        ...
        $('[rel=addUserBtn]').click(function(){
            ...
            var type = $.trim(_this.attr("type"));//1表示请求的是分组 0为班级 
            var boxsrs = $('#popMember_'+sid+'_'+type+'_'+cid); 
            if(boxsrs.find('li').length==boxsrs.find('li.checked').length&&boxsrs.find('li.checked').length!=0){
                $('#clear_'+sid+'_'+type+'_'+cid).attr('tip','1').text('全部取消');
            }else{
                $('#clear_'+sid+'_'+type+'_'+cid).attr('tip','0').text('全部选中'); 
            }
        ...
        $('.selectClassListBox').change(function(){ 
            ...
            $(".ulListBox").hide();
            $('#popMember_'+sid+'_'+type+'_'+cid).show();
            if(tip=='0'){ 
                if(cid){ 
                    ajaxUserPost(cid,sid,cname,sname,type); 
                } 
            }
            $('[rel=allCheck]').hide();
            $('#clear_'+sid+'_'+type+'_'+cid).show();
            _this.attr('tip','1');
            var boxsrs = $('#popMember_'+sid+'_'+type+'_'+cid); 
            if(boxsrs.find('li').length==boxsrs.find('li.checked').length&&boxsrs.find('li.checked').length!=0){
                $('#clear_'+sid+'_'+type+'_'+cid).attr('tip','1').text('全部取消');
        ...
        function ajaxUserPost(cid,sid,cname,sname,type){ 
            var url="<?php echo Yii::app()->createUrl('xiaoxin/notice/getteacher');?>";
                ...
                if(data&&data.status==='1'){ 
                    ...
                    if(data&&data.data){ 
                        $.each(data.data,function(i,v){
                            ...
                            if(type=='dept'){
                                str.push('<li><a href="javascript:void(0);" class="addStudent_'+v.userid+'" type="dept" id="addStudent_'+(type=='group'? v.member:v.userid)+'" sname='+ v.name+' rel="chekedItime" tid="0" pid="'+ (v.userid? v.userid: v.member)+'" data-sid="'+sid+'" data-userid="'+ (v.userid? v.userid: v.member)+'" data-cid="'+cid+'" data-tpyid="0">'+ (name.length>1?name[1]: v.name)+'</a></li>');
                            }else{
                                str.push('<li><a href="javascript:void(0);" class="addStudent_'+v.userid+'" type="group" id="addStudent_'+(type=='group'? v.member:v.userid)+'" sname='+ v.name+' rel="chekedItime" tid="0" pid="'+ (v.userid? v.userid: v.member)+'" data-sid="'+sid+'" data-userid="'+ (v.userid?v.userid:v.member)+'" data-cid="'+cid+'" data-tpyid="0">'+ (name.length>1?name[1]: v.name)+'</a></li>');
                            } 
                        });
                    } 
                    if(str.join('').length){
                        $('#popMember_'+sid+'_'+type+'_'+cid).html(str.join(''));
                    }else{
//                        if(type=='dept'){
//                            $('#popMember_'+sid+'_'+type+'_'+cid).html('<li><a href="javascript:void(0);" style="color:#999;">该部门还没有添加老师</a></li>');
//                        }else{
//                            $('#popMember_'+sid+'_'+type+'_'+cid).html('<li><a href="javascript:void(0);" style="color:#999;">该老师分组还没有添加老师</a></li>');
//                        }
                    } 
                }
            });
        ...
        //请求学生数据
        $(".selectClass1").live('click',function(){
                ...
                if(data&&data.status==='1'){
                    ...
                    if(data&&data.data){ 
                        $.each(data.data,function(i,v){
                            var name= v.name.split(":");
                            if(type=='dept'){
                                str.push('<li severType="0"><a href="javascript:void(0);" id="addStudent_'+(type=='group'? v.member:v.userid)+'" sname='+ v.name+' rel="addStudent" tid="0" pid="'+ (v.userid? v.userid: v.member)+'" data-sid="'+sid+'" data-userid="'+ (v.userid? v.userid: v.member)+'" data-cid="'+cid+'" data-tpyid="0"><span class="fright ">✔</span>'+ (name.length>1?name[1]: v.name)+'</a></li>');
                            }else{
                                str.push('<li severType="0"><a href="javascript:void(0);" id="addStudent_'+(type=='group'? v.member:v.userid)+'" sname='+ v.name+' rel="addStudent" tid="0" pid="'+ (v.userid? v.userid: v.member)+'" data-sid="'+sid+'" data-userid="'+ (v.userid?v.userid:v.member)+'" data-cid="'+cid+'" data-tpyid="0"><span class="fright ">✔</span>'+ (name.length>1?name[1]: v.name)+'</a></li>');
                            } 
                        });
...
//                            $('#popMember_'+sid+'_'+type+'_'+cid).html('<li><a href="javascript:void(0);" style="color:#999;">该老师分组还没有添加老师</a></li>');
        ...
        $('[rel=chekedItime]').live('click',function(){ 
            ...
            type = selectClassListBoxs.attr('type'),
            ...
            if(parseInt(tid)==0){
                ...
                var box = $('#popMember_'+sids+'_'+type+'_'+cid);
                //alert(box.find('li').length+'=='+box.find('li.checked').length);
                ...
                if(data&&data.status==='1'){
                    ...
                    if(data&&data.data){ 
                        $.each(data.data,function(i,v){
                            var name= v.name.split(":");
                            if(type=='dept'){
                                str.push('<li><a href="javascript:void(0);" id="addStudent_'+(type=='group'? v.member:v.userid)+'" sname='+ v.name+' rel="addStudent" tid="0" pid="'+ (v.userid? v.userid: v.member)+'" data-sid="'+sid+'" data-userid="'+ (v.userid? v.userid: v.member)+'" data-cid="'+cid+'" data-tpyid="0"><span class="fright ">✔</span>'+ (name.length>1?name[1]: v.name)+'</a></li>');
                            }else{
                                str.push('<li><a href="javascript:void(0);" id="addStudent_'+(type=='group'? v.member:v.userid)+'" sname='+ v.name+' rel="addStudent" tid="0" pid="'+ (v.userid? v.userid: v.member)+'" data-sid="'+sid+'" data-userid="'+ (v.userid?v.userid:v.member)+'" data-cid="'+cid+'" data-tpyid="0"><span class="fright ">✔</span>'+ (name.length>1?name[1]: v.name)+'</a></li>');
                            } 
                        });
                    ...
                    if(str.join('').length){
                        ...
                        if(type=='dept'){
                            $("#studentListS").html('<li><a href="javascript:void(0);" style="color:#999;">该部门还没有添加老师</a></li>');
                        }else{
                            $("#studentListS").html('<li><a href="javascript:void(0);" style="color:#999;">该老师分组还没有添加老师</a></li>');
                        }
                    }
                    $('#studentListS').show();
