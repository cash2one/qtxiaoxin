                        ...
                        <table class="tableForm" id="tableFormAdd">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><div class="addLimit" style="display: none;"><span class="Validform_checktip Validform_wrong" >一次最多只能添加10位老师</span></div></td>
                                    <td></td>
                                </tr>
                                <tr>
                                   <td></td>
                        ...
                        <table class="tableForm" >
                            <tbody> 
                                ...
                                <tr style="display: none;">
                                    <td>
                                        <div class="inputBox"> 
                                            <textarea placeholder="对老师说点什么吧" maxlength="110" datatype="*1-100" nullmsg="信息不能为空！" errormsg="信息不能大于100个字！" style="width: 486px; height: 98px;" name="Teacher[desc]">欢迎加入<?php echo $class->name; ?></textarea>
                                        </div> 
                                ...
                                <tr>
                                    <td> 
                                        <!-- <a class="btn btn-raed" >发送邀请</a>  -->
                                        <input type="button" rel="sendBtn" class="btn btn-raed" value="保 存">
<!--                                        &nbsp;&nbsp;
                                        <a class="btn btn-default" href="<?php echo Yii::app()->createUrl('/xiaoxin/class/teachers/'.$class->cid);?>">返回</a> -->
                                    </td>
                                </tr>
                            </tbody>
                        </table> 
...
$(function() {
    ...
    function addInput(str){
        var itme ='<tr><td style="width:10px;"><div class="inputBox"><input rel="name" name="Teacher[name][]" maxlength="8" class="sm" type="text" placeholder="输入姓名" >'
                +'<br/><span class="ValidTip Validform_checktip" >&nbsp;</span></div></td>'
                +'<td style="width:345px;"><div class="inputBox"><input rel="mobile" name="Teacher[mobile][]" maxlength="11" class="lg" type="text" placeholder="输入电话">'
    ...
    $('#btnAdd').click(function(){
        if(cuntFlag<10){
         ...
         $(".addLimit").hide();
           ...
           $(".addLimit").show();
           $(".addLimit").find('span').text('一次最多只能添加10位老师！');
        }
        AutoHeight(); 
    });
    //删除表单
    $('[rel=btnDele]').live('click',function(){
        ...
        if(cuntFlag>0){ 
            ...
            $(".addLimit").show();
            $(".addLimit").find('span').text('请至少添加一位老师！'); 
        } 
    });
    ...
    $('[rel=sendBtn]').click(function(){
        ...
        inputBox.find('input[rel=mobile]:visible').each(function(k,v){
            ...
            if(relr==""){
                fMobile++;
                $(this).parents('td').find('.tipB').addClass('Validform_wrong').text('电话号码不能为空！');
            }else if(eg.test(relr)&&relr.length==11){  
            }else{
                fMobile++;
                $(this).parents('td').find('.tipB').addClass('Validform_wrong').text('电话号码格式不正确!'); 
            } 
